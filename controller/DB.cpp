...
bool DB::get(const uint64_t networkId,nlohmann::json &network)
{
	...
	{
		zt::shared_lock<std::shared_mutex> l2(nw->lock);
		network = nw->config;
	}
	return true;
}
...
void DB::_memberChanged(nlohmann::json &old,nlohmann::json &memberConfig,bool notifyListeners)
{
	...
	if (old.is_object()) {
		...
		if ((memberId)&&(networkId)) {
			{
				zt::unique_lock<zt::shared_mutex> l(_networks_l);
			...
			if (nw) {
				zt::unique_lock<std::shared_mutex> l(nw->lock);
	...
	if (memberConfig.is_object()) {
		...
		if (notifyListeners) {
			zt::unique_lock<zt::shared_mutex> ll(_changeListeners_l);
...
void DB::_networkChanged(nlohmann::json &old,nlohmann::json &networkConfig,bool notifyListeners)
{
	...
	if (networkConfig.is_object()) {
		...
		if (networkId) {
			...
			{
				zt::unique_lock<std::shared_mutex> l2(nw->lock);
